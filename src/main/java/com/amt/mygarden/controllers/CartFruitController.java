...
public class CartFruitController {
    ...
    public RedirectView addFruitToCart (@PathVariable String fruitId, @ModelAttribute String fruitAmount) throws Exception {
        // @ResponseBody means the returned String is the response, not a view name
        // @RequestParam means it is a parameter from the GET or POST request
        final RedirectView redirectView = new RedirectView("/", true);

        Fruit fruit = fruitService.getASingleFruit(fruitId);

        Optional<Item> it=itemRepository.findUserFruitAndCart("admin",fruit);
        if(it.isPresent()){
            Item item1=it.get();
